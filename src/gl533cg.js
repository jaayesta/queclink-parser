...
const parse = raw => {
  ...
  const deviceTypeStr = deviceTypeInt.toString(16).padStart(4, '0').toUpperCase() // C301
  offset += 2

  // Protocol Version 2 bytes
  const protocolVersionInt = buf.readUInt16BE(offset)
  const protocolVersionStr = protocolVersionInt.toString(16).padStart(4, '0').toUpperCase() // 0005
  offset += 2

  // Custom Version 1 byte
  const customVersion = buf.readUInt8(offset)
  offset += 1
  ...
  let data = {
    ...
    type: 'data',
    imei: imei,
    protocolVersion: utils.getProtocolVersion(`${deviceTypeStr}${protocolVersionStr}`),
    temperature: null,
  ...
  // Records Part
  // Read records
  while (offset < buf.length - 4) { // keep 4 bytes for Count, CRC, Tail
    ...
    while (offset < recordEnd && offset < buf.length - 4) {
      ...
      if (dataId === 0x01) { // Profile ID
        data.profileId = dataContent.readUInt8(0)
      } else if (dataId === 0x02) { // Device Name
        // data.deviceName = dataContent.toString()
        continue
      } else if (dataId === 0x04) { // Device Serial Number
        data.serialNumber = dataContent.toString()
      } else if (dataId === 0x07) { // Current Working Mode
        data.status = data.status || {}
        const mode = dataContent.readUInt8(0)
        data.connectionMode = mode === 0x00 ? 'Power saving' : mode === 0x01 ? 'Continuous' : 'Unknown'
      } else if (dataId === 0x0A) { // Internal Battery Percentage
        data.voltage = data.voltage || {}
        data.voltage.battery = dataContent.readUInt8(0)
        data.voltage.inputCharge = false
      } else if (dataId === 0x11) { // RF433 Working Status
        const rfState = dataContent.readUInt8(0)
        data.rf433 = rfState === 0 ? 'Normal' : rfState === 1 ? 'Module abnormal' : rfState === 2 ? 'Antenna abnormal' : 'Unknown'
      } else if (dataId === 0x13) { // Triggered Time
        const triggeredTime = dataContent.readUInt32BE(0)
        data.datetime = new Date(triggeredTime * 1000)
      } else if (dataId === 0x16) { // Record Count (History count)
        data.recordCount = dataContent.readUInt16BE(0)
      } else if (dataId === 0x17) { // Motion Status
        const motion = dataContent.readUInt8(0)
        data.status = data.status || {}
        data.status.motion = motion === 0x00 ? 'Motionless' : motion === 0x01 ? 'Moving' : 'Unknown'
      ...
      //   data.alarm = utils.getAlarm('GTINFO', null)
      } else if (dataId === 0x2B) { // Light Sensor
        data.lightSensor = dataContent.readUInt8(0) === 0x01 ? 'Triggered' : 'Normal'
      } else if (dataId === 0x50) { // Fixed Report Information
        data.alarm = utils.getAlarm('GTFRI', null)
      } else if (dataId === 0x51) { // 81 Mini Location
        let locOffset = 0
        ...
        data.gpsSignalStrength = utils.getSignalStrength('GPS', signalStrength, true)
        data.fixState = fixState === 0 ? 'Off' : fixState === 1 ? 'No fix' : fixState === 2 ? 'Fix' : 'Unknown'
        data.fixMode = fixMode === 0 ? '2D' : fixMode === 1 ? '3D' : 'Unknown'

      } else if (dataId === 0x52) { // 82 Full Location
        let locOffset = 0
        const fixStateMode = dataContent.readUInt8(locOffset)
        const signalStrength = (fixStateMode >> 4) & 0x0F
        const fixState = (fixStateMode >> 2) & 0x03
      ...
      } else if (dataId === 0x55) { // 85 Registered Cell
        const plmnLen = dataContent.readUInt8(0)
        const plmnVal = (dataContent.readUInt8(1) << 16) | (dataContent.readUInt8(2) << 8) | dataContent.readUInt8(3)
        ...
        data.accessTechnology = tech === 0 ? 'GSM' : tech === 5 ? 'LTE Cat-1' : 'Unknown'
      ...
      } else if (dataId === 0x58) { // 88 SIM Card
        data.imsi = dataContent.slice(0, 8).toString('hex').replace(/^0/, '').replace(/f$/i, '')
      ...
      } else if (dataId === 0x5A) { // 90 Versions
      ...
      } else if (dataId === 0x5C) { // 92 GEO Status
        data.geofence = data.geofence || {}
        data.geofence.id = dataContent.readUInt8(0)
        const gStat = dataContent.readUInt8(1)
        data.geofence.status = gStat === 0x00 ? 'Inside' : gStat === 0x01 ? 'Outside' : 'Unknown'
      } else if (dataId === 0x5D) { // 93 All GEO Status
        const geofenceCount = dataLen / 2
        data.geofences = []
        for (let i = 0; i < geofenceCount; i++) {
          data.geofences.push({
            id: dataContent.readUInt8(i * 2),
            status: dataContent.readUInt8(i * 2 + 1) === 0x00 ? 'Inside' : 'Outside'
          })
        }
      } else if (dataId === 0x65 || dataId === 0x66 || dataId === 0x67) { // 101/102/103 Upgrade/Update/Get Config
        const getUpgradeStatus = (c, sc) => {
          const codes = {
            1: 'Start download', 2: 'Start update', 3: 'Download successful',
            4: 'Download failed', 5: 'Update successful', 6: 'Update failed'
          }
          const subcodes = {
            0: 'Normal', 16: 'Low battery', 32: 'No network',
            33: 'Connection failed', 48: 'File not found', 49: 'CRC error',
            50: 'Type error', 51: 'Size error', 52: 'Incompatible version',
            240: 'In progress', 255: 'Unknown'
          }
          return {
        ...
        const status = getUpgradeStatus(dataContent.readUInt8(0), dataContent.readUInt8(1))
        const fieldName = dataId === 0x65 ? 'upgradeInfo' : dataId === 0x66 ? 'updateConfig' : 'getConfig'
        data[fieldName] = {
          status: status.code,
          reason: status.subcode,
          sequence: dataContent.readUInt16BE(5)
        }
      } else if (dataId === 0x61) { // 97 Internal Battery Status
        data.voltage = data.voltage || {}
      ...
      } else if (dataId === 0x78) { // 120 Self Test
      ...
      } else if (dataId === 173) { // 80ADH SVR Status (ID 173 dec)
        const appendInfo = dataContent.slice(8).toString('hex') !== '00'
        data.bluetooth = data.bluetooth || {}
        data.bluetooth.connection = dataContent.readUInt8(0) === 0x01 ? 'Normal' : 'Abnormal'
        data.bluetooth.role = dataContent.readUInt8(1) === 0x00 ? 'Master' : 'Slave'
        data.bluetooth.targetMac = dataContent.slice(2, 8).toString('hex').toUpperCase()
        if (appendInfo) {
          const lostReasonHex = dataContent.slice(8, dataContent.length - 1).toString('hex')
          data.bluetooth.lostReason = lostReasonHex === '03' ? 'Bluetooth connection failed' :
            lostReasonHex === '07' ? 'Bluetooth connection successfull' :
              lostReasonHex === '08' ? 'Failed to decrypt interactive data' :
                lostReasonHex === '09' ? 'Data check is incorrect (after decryption)' :
                  lostReasonHex === '0B' ? 'No data interaction (after connection)' :
                    'Unknown'
        }
      } else if (dataId === 159) { // 809FH Bluetooth Info (ID 159 dec)
        data.bluetooth = data.bluetooth || {}
        const nameLength = dataContent.readUInt8(0)
        data.bluetooth.name = dataContent.slice(1, nameLength + 1).toString()
        data.bluetooth.mac = dataContent.slice(nameLength + 1, nameLength + 7).toString('hex').toUpperCase()
      }
    }
  }
